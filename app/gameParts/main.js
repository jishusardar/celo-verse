...
export function useGame(canvasRef) {
  ...
  useEffect(() => {
    ...
    let spriteTiles = new Tileset(playerSprite, 16, 16, 3, ctx);
    let newMapSet = new Tileset(mapSprites, 8, 8, 49, ctx);
    let logoTile = new Tileset(logo, 16, 16, 3, ctx);
    let fontTile = new Tileset(font, 8, 8, 39, ctx);
    let soundHandler = new Sound();
    let walkSpeed = 100;

    let spriteDownAnim = new Animation(spriteTiles, ['1,2', '0,2', '2,2'], walkSpeed);
    let spriteLeftAnim = new Animation(spriteTiles, ['0,0', '1,0', '2,0'], walkSpeed);
    let spriteRightAnim = new Animation(spriteTiles, ['0,3', '1,3', '2,3'], walkSpeed);
    let spriteUpAnim = new Animation(spriteTiles, ['0,1', '1,1', '2,1'], walkSpeed);
    let keysDown = {};

    let ground = level.layers[0].data;
    let collision = [];
    let objects = {};
    let player = {};
    let entities = [];
        ...
        if (!entity.collected && overlap(player.x, player.y, 32, 32, entity.x + 8, entity.y + 8, 16, 16)) {
          switch (entity.type) {
            ...
            case 'floor':
              ...
              const doors = entities.filter(function (obj) {
    ...
    function drawObjects() {
      for (let i = 0; i < entities.length; i++) {
        if (!entities[i].collected) {
          const posX = entities[i].x - game.viewport.x;
          const posY = entities[i].y - game.viewport.y;
          const assetX = entities[i].gid * 8;
    ...
    function drawMenu() {
      for (let i = 0; i < menuMap.length; i++) {
        const posX = (i % 8) * 32;
        const posY = Math.floor(i / 8) * 32;
        let assetX = (menuMap[i] - 1) * 8;
    ...
    if (isMobile()) {
      ...
      if (mobileControl) {
        mobileControl.addEventListener('touchstart', handleTouchStart);
        mobileControl.addEventListener('touchend', handleTouchEnd);
      }
    }
    ...
    return () => {
      ...
      const mobileControl = document.getElementById('mobile-control');
      if (mobileControl) {
        mobileControl.removeEventListener('touchstart', () => {});
        mobileControl.removeEventListener('touchend', () => {});
      }
    };
  }, [canvasRef]);
}